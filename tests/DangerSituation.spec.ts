...
test("Performance DangerSituation Page - Concurrent", async () => {
  ...
  console.log(
    `1 Run = 1 Browser | เปิดพร้อมกัน ${DANGER_PERFORMANCE_DATA.PROVINCE_IDS.length} Contexts`,
  );

  for (let run = 1; run <= DANGER_PERFORMANCE_DATA.TOTAL_RUNS; run++) {
    console.log(`================ Run ${run} ================`);

    const dangerPage = new DangerPerformancePage();

    try {
      await dangerPage.openBrowser();

      const tasks: Promise<PerformanceResult>[] =
        DANGER_PERFORMANCE_DATA.PROVINCE_IDS.map(async (provinceId) => {
          try {
            const path = DANGER_PERFORMANCE_DATA.PATH_TEMPLATE.replace(
              "{provinceId}",
              String(provinceId),
            );
  ...
  const totalExpected =
    DANGER_PERFORMANCE_DATA.TOTAL_RUNS *
    DANGER_PERFORMANCE_DATA.PROVINCE_IDS.length;
