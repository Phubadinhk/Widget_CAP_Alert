...
test("Performance DangerSituation Page", async () => {
  ...
  console.log(
    `Failed Runs: ${errorLogs.length}/${DANGER_PERFORMANCE_DATA.TOTAL_RUNS}`,
  );
  ...
  if (errorLogs.length > 0) {
    throw new Error(
      `Performance test failed: ${errorLogs.length}/${DANGER_PERFORMANCE_DATA.TOTAL_RUNS} runs failed. Please check Error Summary above.`,
    );
  }
});
