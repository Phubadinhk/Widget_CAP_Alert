...
test("Performance WeatherByProvincePage - Concurrent", async () => {
  ...
  console.log(
    `1 Run = 1 Browser | เปิดพร้อมกัน ${WEATHER_PERFORMANCE_DATA.PROVINCE_IDS.length} Contexts`,
  );

  for (let run = 1; run <= WEATHER_PERFORMANCE_DATA.TOTAL_RUNS; run++) {
    console.log(`================ Run ${run} ================`);

    const weatherPage = new WeatherPerformancePage();

    try {
      await weatherPage.openBrowser();

      const tasks: Promise<PerformanceResult>[] =
        WEATHER_PERFORMANCE_DATA.PROVINCE_IDS.map(async (provinceId) => {
          try {
            const path = WEATHER_PERFORMANCE_DATA.PATH_TEMPLATE.replace(
              "{provinceId}",
              String(provinceId),
            );
            ...
            const finishTimeSec =
              await weatherPage.gotoRootThenTargetAndGetNetworkFinishTimeByNewContext(
                ...
                `reports/screenshots/WeatherByProvince/run-${run}-province-${provinceId}.png`,
              );
  ...
  const totalExpected =
    WEATHER_PERFORMANCE_DATA.TOTAL_RUNS *
    WEATHER_PERFORMANCE_DATA.PROVINCE_IDS.length;
